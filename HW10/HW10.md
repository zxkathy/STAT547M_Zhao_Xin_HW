...
df <- c()
for(i in 1:length(h)){
    info[[i]] <- html_nodes(h[[i]], css = "dd")
    info_text <- html_text(info[[i]])
    ...
    df <- bind_rows(df, pageHouse %>%na.omit())
}
...
After renaming the data frame, we got 90 rows, which means we got 90 house info. Let's have a look at the head part of data frame.

Before using the data frame `df` doing any analysis, we need to change to character columns to number columns, such as `Price`, `Beds`, `Baths`
...
house_dat <- 
    df %>%
    mutate(BedN = as.numeric(df$Beds),
                 BathN = as.numeric(df$Baths),
                 PriceChar = unlist(map(df$Price, remDol)), 
...
| MLS® \#  |  PriceInK|  BedN|  BathN| Type                 | Address                       | Subdivision | Area           |
|:---------|---------:|-----:|------:|:---------------------|:------------------------------|:------------|:---------------|
| R2205230 |     12888|     3|      4| Residential Attached | \# 4701 938 Nelson Street     | Downtown VW | Vancouver West |
| R2208011 |      4989|     4|      4| Residential Attached | \# 1602 667 Howe Street       | Downtown VW | Vancouver West |
| R2134299 |      4840|     3|      5| Residential Attached | \# 5001 1480 Howe Street      | Downtown VW | Vancouver West |
| R2225747 |      4588|     4|      4| Residential Attached | \# 1403 1335 Howe Street      | Downtown VW | Vancouver West |
| R2226517 |      4584|     2|      3| Residential Attached | \# Ph9 777 Richards Street    | Downtown VW | Vancouver West |
| R2210404 |      4480|     3|      4| Residential Attached | \# 2601 885 Cambie Street     | Downtown VW | Vancouver West |
| R2187335 |      4398|     2|      3| Residential Attached | \# 2402 1335 Howe Street      | Downtown VW | Vancouver West |
| R2224334 |      4318|     2|      3| Residential Attached | \# 3504 838 W Hastings Street | Downtown VW | Vancouver West |
| R2207903 |      4070|     3|      3| Residential Attached | \# Ph2 777 Richards Street    | Downtown VW | Vancouver West |
| R2163601 |      3780|     3|      3| Residential Attached | \# 4905 777 Richards Street   | Downtown VW | Vancouver West |
